...
if __name__ == "__main__":
    ...
    # 1. Chargement du dataset existant et extraction des IDs
    if os.path.exists(OUTPUT_FILE):
        try:
            df_existing = pd.read_csv(OUTPUT_FILE)
            # On convertit en set d'entiers pour une recherche ultra-rapide (O(1))
            existing_ids = set(df_existing['id_sortie'].astype(int).unique())
        ...
        df_existing = None
        existing_ids = set()
        print(f"\n📂 Nouveau dataset (le fichier sera créé)")
    
    # 2. Récupération des sorties via l'API
    print("\n📡 Récupération des dernières sorties Skitour...")
    data = fetch_sorties(nb=700, year=2023)
    
    if not data:
        print("❌ Impossible de récupérer les sorties. Vérifiez votre clé API ou connexion.")
        exit(1)
    
    SKIABILITE_MAP = {
        '1': 'Mauvaise', '2': 'Médiocre', '3': 'Correcte', 
    ...
    # 3. Filtrage rigoureux des doublons
    for sortie in data:
        try:
            sortie_id = int(sortie.get("id"))
        except (TypeError, ValueError):
            continue
            
        # VERIFICATION : On ignore si l'ID est déjà présent dans le fichier CSV
        if sortie_id in existing_ids:
            skipped += 1
            continue  # On passe à la suivante sans rien faire
        
        # --- Traitement d'une nouvelle sortie ---
        
        sommets_list = sortie.get("sommets", [])
        if not sommets_list:
            # Si pas de sommet, on passe à la sortie suivante ou on définit un sommet vide
            print(f"⚠️ Sortie {sortie_id} ignorée : pas de sommet associé.")
            continue 
        sommet = sommets_list[0]
    ...
    # 4. Décision de continuer ou non
    if not results:
        print(f"\n✅ Terminé : Les {skipped} sorties récupérées sont déjà dans votre CSV.")
        exit(0)
    
    print(f"📊 {len(results)} nouvelles sorties identifiées (ignoré {skipped} déjà traitées)")
    
    df_new = pd.DataFrame(results)
    
    print("\n" + "=" * 60)
    print("🚀 Enrichissement ML (Uniquement pour les nouvelles sorties)")
    print("=" * 60)
    
    try:
        # L'enrichissement ne se fait QUE sur les nouvelles lignes
        df_ml_new = enrich_with_ml_features(df_new, use_llm=False)
        
        # 5. Fusion finale avec l'ancien dataset
        if df_existing is not None:
            print(f"\n🔗 Fusion des nouvelles données avec l'historique...")
        ...
        print(f"\n✅ Dataset mis à jour : {OUTPUT_FILE}")
        print(f"📈 Total final : {len(df_final)} sorties enregistrées")
        
    except Exception as e:
        print(f"\n❌ ERREUR lors de l'enrichissement : {e}")
        print("💡 Les caches partiels sont sauvegardés. Relancez le script pour reprendre.")