...
MAX_REQUESTS_PER_DAY = 400  # Limite de sécurité (sur 1000)
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if len(days) > 1 and days_before > 0:
                ...
                tmax = [d.get("tempmax") for d in history_days if d.get("tempmax") is not None]
                tmin = [d.get("tempmin") for d in history_days if d.get("tempmin") is not None]
                winds = [d.get("windspeed") for d in history_days if d.get("windspeed") is not None]
                snows = [d.get("snow", 0) for d in history_days]
                ...
                # Moyennes simples
                result["temp_max_7d_avg"] = float(sum(tmax) / len(tmax)) if tmax else None
                result["temp_min_7d_avg"] = float(sum(tmin) / len(tmin)) if tmin else None
                
                # Amplitude thermique moyenne
                if tmax and tmin:
                    result["temp_amp_7d_avg"] = float(sum((tx - tn) for tx, tn in zip(tmax, tmin) if tx is not None and tn is not None)/ len(tmax))
                else:
                    result["temp_amp_7d_avg"] = None
                
                # Cycles gel / dégel
                freeze_thaw = 0
                for d in history_days:
                    if d.get("tempmin") is not None and d.get("tempmax") is not None:
                        if d["tempmin"] < 0 < d["tempmax"]:
                            freeze_thaw += 1
                
                result["freeze_thaw_cycles_7d"] = freeze_thaw
                
                # Vent max sur 7 jours
                result["wind_max_7d"] = max(winds) if winds else None
                
                # Neige cumulée
                result["snowfall_7d_sum"] = float(sum(snows) * 10)
                
                # Jours depuis dernière chute
                result["days_since_last_snow"] = None
...
def get_meteo_forecast(lat, lon, date_str, horizon_days=1):
    """
    Récupère la météo prévue (forecast) pour J0 ou J+1
    """
    url = "https://weather.visualcrossing.com/VisualCrossingWebServices/rest/services/timeline"

    try:
        target_date = datetime.strptime(date_str, "%Y-%m-%d") + timedelta(days=horizon_days)
    except:
        print(f"❌ Date forecast invalide: {date_str}")
        return None

    endpoint = f"{url}/{lat},{lon}/{target_date.strftime('%Y-%m-%d')}"

    params = {
        "key": VISUAL_CROSSING_KEY,
        "unitGroup": "metric",
        "include": "days",
        "elements": "datetime,tempmax,tempmin,windspeed,snow"
    }
    ...
    try:
        ...
        if response.status_code != 200:
            print(f"⚠️ Forecast HTTP {response.status_code}")
            return None
        ...
        return {
            "temp_max_forecast": d.get("tempmax"),
            "temp_min_forecast": d.get("tempmin"),
            "wind_forecast": d.get("windspeed"),
            "snowfall_forecast": (d.get("snow", 0) * 10)
        }

    except Exception as e:
        print(f"❌ Erreur forecast météo: {e}")
        return None
...
if __name__ == "__main__":
    ...
    try:
        ...
        output_file = "skitour_ml_dataset_chat.csv"
